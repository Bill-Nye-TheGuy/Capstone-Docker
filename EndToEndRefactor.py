...
class ModelAssets:
    ...
    def feature_extraction(self,filename, accent = None, sampling_rate=48000):
        ...
        #Calculate Features
        spectral_centroid = np.mean(ft.spectral_centroid(y=audio, sr=sampling_rate))
        spectral_bandwidth = np.mean(ft.spectral_bandwidth(y=audio, sr=sampling_rate))
        spectral_rolloff = np.mean(ft.spectral_rolloff(y=audio, sr=sampling_rate))
        features.append(accent)
        features.append(spectral_centroid)
        features.append(spectral_bandwidth)
        features.append(spectral_rolloff)
        mfcc = ft.mfcc(y=audio, sr=sampling_rate,n_mfcc = 12, lifter = 5)
